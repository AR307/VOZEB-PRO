...
- [安全] 管理员支付、用户、设置、渠道密钥、对象存储与业务备份操作统一逐次验证当前密码，已启用 MFA 时同时验证动态码；备份导出在验证后才读取脱敏数据，登录成功新增环境变化提醒和本人分页登录记录，证明字段不进入业务 payload、全局状态或审计。
- [界面] 后台渠道详情、接入向导和逻辑模型 Drawer 使用 Ant Design 6 响应式 wrapper 约束，390px/430px 不再保持桌面宽度或向左裁切，浅深主题复核弹窗保持可读。
...
- [工程] 发布门禁通过 442 个 Web 测试文件、1869 项测试、0 已知依赖漏洞、62 页生产构建及桌面/390px/430px 共 74 项 Playwright 回归；真实 PostgreSQL 支付、Docker、支付商和外部渠道继续保留为发布环境验收项。
