...
- [存储] 阻止外部存储 WebP 预览再次生成嵌套预览，内部预览不再进入文件列表；同一源图与规范化宽度在所有页面复用同一个预览对象。后台新增历史异常预览清理，只删除递归变体并保留原图与正常首层预览。对象存储遗漏图片 MIME、返回 `application/octet-stream` 时改按真实字节识别 WebP，避免参考素材被误判为格式错误。
- [品牌] 统一创作、Canvas、短剧 Agent 及提示词优化、创作质检、Skill 导入的运行时身份改为读取后台网站标题；开源项目名称、默认兜底和备份协议标识保持不变。
- [生成] 统一创作图片参数增加 1:1、16:9、9:16 的 2K/4K 明确像素预设；预设沿用精确宽高链路，自定义编辑器不会误展开，智能比例仍保持自动判断。
- [Canvas] 收敛智能分层链路：视觉识别只执行一次，每个元素都以完整源图分别提交透明图片任务并落成可移动、可保存、可下载的独立节点，不再使用本地切割冒充分层结果；独立消除背景继续使用本地主体分割，背景补全沿用独立蒙版流程。
- [生成/Canvas] 待检查任务不再要求用户等待管理员；图片、视频、文本和音频均可用原上游任务 ID 主动检查一次，结果已返回时继续落盘，仍在处理时保留“检查状态”入口，不重复创建或计费；声明式图片协议恢复轮询继续使用站内系统代理，不会被响应中的真实媒体基址错误替换。
- [Canvas] 修复图片节点直接生成视频时漏传当前图片的问题；普通图生视频固定使用当前节点作为唯一主图引用并保存到输出快照，不再退化为文生视频或误用更早的画布图片，失败重试继续沿用原引用。
- [视频/Canvas] WebP 仅用于界面预览，图生视频直接复用去除预览参数的原始媒体，不再回读后重复上传；PNG/JPEG 源文件保持原地址，真实 WebP 源由服务端无缩放转为临时 PNG 后提交上游。修复 Canvas Agent 长提示词包含 `@图片` 时高亮层越过输入框、把文字顶到消息区的问题。
- [生成/Canvas] 修复智能比例和固定尺寸优先级：明确选择的 Canvas 尺寸不会被参考图或规划比例覆盖，`auto` 保持智能判断，不再被统一改写为 `1:1` 或其他固定比例。
- [/create] 收紧输入框收起态为稳定的 60px 外壳和 44px 输入行，回到底部自动恢复展开；滚动方向状态不再因临界值反复弹跳，收起时隐藏附加工具行且发送按钮保持可见。

## v0.0.7

- [计费/图片] 图片编辑从 multipart 回退到 JSON、切换响应格式或切换 Responses 协议时使用独立上游尝试幂等键，避免失败请求退款后复用原消费记录导致成功回退请求未计费。
- [短剧] 支持导入 DOCX/Word 文档并按真实章节拆分为剧集；集数可在项目设置中修改并持久化，分析、审核、分镜和镜头生成按当前集数据恢复。
- [短剧/生成] 收紧小说分析与分镜数据链路，保留人物对白、来源文本和镜头边界；结构化上游返回异常时按公开响应内容解析并保留可定位的失败状态，不用不完整占位数据冒充成功。
- [协议] 自定义协议文档解析改为面向任意文档地址提取目录、创建、查询和结果协议；支持 `curl --url` 示例、按路径边界匹配自定义端点，并使用本地任意路径文档 fixture 验证图片与视频全链路。
- [Canvas/性能] 画布大节点集加载、拖拽、缩放、网格绘制和保存链路完成大规模回归，10,001 个节点与 10,000 条连线可打开并交互；批量下载在桌面和移动端均按真实选区生成 ZIP；节点提示词浮层按画布可视区域夹紧，缩放或窄屏时关闭与放大按钮不再移出视口。
- [媒体/删除] 对话、Canvas 和短剧删除改为按用户、实体及指定媒体键定向清理；只有业务引用归零时删除本地或 OSS 原件，并同步清理 WebP 变体，缺失引用按不可用媒体隐藏而不是渲染错误。
- [上游/测试] 新增本地自建协议后台覆盖文本、图片、视频、查询和结果字段，统一 Agent、Canvas 与短剧入口的协议回归不依赖外部供应商。
- [Canvas] 新增本地通用主体分割、透明去背、智能分层、人脸识别与表情参考；分层背景通过现有图片编辑任务补全，失败后保留主体和稳定蒙版用于原位重试；透明主体仅在画布预览棋盘格，原始下载保持透明。
- [Canvas] 框选多张图片或视频后可在浏览器内存中打包下载 ZIP，不占用服务器归档空间；导出能力保留原图和批量 ZIP，不再提供 PSD。
- [Canvas] 一键整理按独立生成分支、拓扑层与父子重心排版，循环和多父节点重复整理保持稳定；Agent 内部任务节点默认隐藏，真实连线不再穿过无关节点或与其他分支交叉。
- [生成] 图片上游一次返回多份结果时完整保存并返回，图片工作台、Canvas、短剧与 Agent 共用任务入口继续保留全部成功结果；对象存储失败提示更加明确。
- [生成/Canvas] `submission_outcome_unknown` 统一表示上游提交结果未知且未取得可查询任务 ID；系统停止自动重试以避免重复生成和扣费，Canvas 节点内保留一次可操作说明且不再叠加重复 Toast。
- [后台/性能] 模型渠道添加和删除成功后立即同步页面状态，失败时按并发编辑安全回滚；Canvas 项目在指针或键盘意图时预加载，并并行恢复节点与 Agent 图片，降低进入和切换卡顿。
