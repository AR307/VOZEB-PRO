...
- [Agent] Agent Run 公开 API、SSE 快照和 Canvas 操作改为显式白名单，内部规划上下文、渠道/上游审计、积分流水、复盘和执行提示词只保留在服务端；规划输入按真实 JSON 序列化动态收敛并记录保留/省略身份，复盘建议不会自动重新创建媒体任务。
...
- [工程] 发布门禁通过 455 个 Web 测试文件、4 个按环境跳过，1967 项测试通过、9 项跳过，依赖审计为 0 个已知漏洞，62 页生产构建及桌面/390px/430px 共 74 项 Playwright 回归；真实 PostgreSQL 支付、Docker、支付商和外部渠道继续保留为发布环境验收项。
