...
export function canvasImageDecompositionInstruction(width: number, height: number) {
    return [
        ...
        "文字、Logo、角标和装饰不能因为不是主商品而省略；不要把整张海报或整片前景合并成一个主体框。视觉上独立的元素分别返回，必须一起移动的组合可以保留为一层。",
        "bbox 要完整包住元素且不要裁掉边缘；背景本身不放入 layers。zIndex 从后到前递增。backgroundDescription 只简要描述移除全部前景后应保留的背景。",
