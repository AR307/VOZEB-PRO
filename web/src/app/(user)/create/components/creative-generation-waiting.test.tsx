...
describe("creative generation waiting", () => {
    it("uses the real task phase before elapsed-time comfort copy", () => {
        expect(creativeGenerationWaitingCopy({ mode: "image", runStatus: "planning", progressText: "正在理解需求并选择合适的创作能力", elapsedSeconds: 180 })).toContain("接住你的灵感");
    ...
    it("adapts the comfort copy by media type and natural elapsed minutes", () => {
        ...
        expect(creativeGenerationWaitingCopy({ mode: "video", runStatus: "running", progressText: "仍在上游处理中", elapsedSeconds: 60 })).toContain("一直陪你等着");
        expect(creativeGenerationWaitingCopy({ mode: "video", runStatus: "running", progressText: "仍在上游处理中", elapsedSeconds: 120 })).toContain("久等了");
        expect(creativeGenerationWaitingCopy({ mode: "video", runStatus: "running", progressText: "仍在上游处理中", elapsedSeconds: 180 })).toContain("不用重复发送");
    });
