...
type CanvasAgentMediaCapability = Extract<MediaCapability, "image" | "video">;
type CanvasAgentPreferencePatch = CreativeGenerationPreferencePatch;

export function CanvasAgentGenerationSettings({ preferences, onChange }: { preferences: CreativeGenerationPreferences; onChange: (preferences: CreativeGenerationPreferences) => void }) {
    const capability: CanvasAgentMediaCapability = preferences.mode === "video" ? "video" : "image";
    ...
    return (
        <GenerationPreferencesControl
            ...
            triggerLabel={canvasAgentCompactPreferenceSummary(capability, preferences)}
            triggerAriaLabel={`生成参数：${canvasAgentPreferenceSummary(capability, preferences)}`}
            ...
            onChange={(patch) => onChange(updateCanvasAgentGenerationPreferences(preferences, capability, patch))}
        />
    );
}

export function canvasAgentCompactPreferenceSummary(capability: CanvasAgentMediaCapability, preferences: CreativeGenerationPreferences) {
    if (capability === "video") {
        const video = preferences.video;
        const size = video?.size && video.size !== "auto" ? video.size.replace("x", "×") : "智能";
        if (isExactSize(video?.size)) return size;
        return `${size} · ${video?.seconds || 5}秒`;
    }
    const image = preferences.image;
    const size = image?.size && image.size !== "auto" ? image.size.replace("x", "×") : "智能";
    if (isExactSize(image?.size)) return size;
...
function canvasAgentPreferenceSummary(capability: CanvasAgentMediaCapability, preferences: CreativeGenerationPreferences) {
    if (capability === "video") {
        const video = preferences.video;
        const size = video?.size && video.size !== "auto" ? video.size.replace("x", "×") : "智能";
        const quality = video?.quality && video.quality !== "auto" ? `${video.quality.replace(/p$/i, "")}P` : "智能";
        if (size === "智能" && quality === "智能") return "智能参数";
        return `${size} · ${quality} · ${video?.seconds || 5}秒`;
    }
    const image = preferences.image;
    const size = image?.size && image.size !== "auto" ? image.size.replace("x", "×") : "智能";
    const quality = ({ high: "高", medium: "中", low: "低", auto: "智能" } as const)[image?.quality || "auto"];
    if (size === "智能" && quality === "智能" && (image?.count || 1) === 1) return "智能参数";
    return `${size} · ${quality}${(image?.count || 1) > 1 ? ` · ${image?.count}张` : ""}`;
}

export function updateCanvasAgentGenerationPreferences(preferences: CreativeGenerationPreferences, capability: CanvasAgentMediaCapability, patch: CanvasAgentPreferencePatch): CreativeGenerationPreferences {
    if (capability === "image") {
        const image = { ...preferences.image };
