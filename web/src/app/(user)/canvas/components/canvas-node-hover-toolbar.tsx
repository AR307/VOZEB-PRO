...
export function CanvasNodeHoverToolbar({
    ...
    const nodeToolbarTools: ToolbarTool[] = [
        ...
        ...(hasImage || hasVideo || hasAudio
            ? [{ id: "download", title: node.metadata?.imageLayers?.length ? "下载全部分层" : hasAudio ? "下载音频" : hasVideo ? "下载视频" : "下载图片", label: "下载", icon: <Download className="size-4" />, onClick: () => onDownload(node) }]
            : []),
