...
import { App, Button, Drawer, Image, Input, InputNumber, Popconfirm, Tooltip } from "antd";
...
export function DramaAssetEditorDrawer({ project, kind, assetId, open, onClose }: { project: DramaProject; kind: DramaAssetKind; assetId?: string; open: boolean; onClose: () => void }) {
    ...
    return (
        <Drawer
            title={asset ? `编辑${definition.title}` : `新建${definition.title}`}
            placement="right"
            size={620}
            open={open}
            destroyOnHidden
        ...
        >
            <div className="grid gap-6 p-4 sm:p-6">
                <section className="grid min-w-0 gap-4 sm:grid-cols-[136px_minmax(0,1fr)]">
                    <div className="grid aspect-[4/5] w-full place-items-center overflow-hidden rounded-xl border border-border bg-muted/50">
                        {primary?.url ? (
                            ...
                            <div className="grid gap-2 text-center text-muted-foreground">
                                ...
                                <span className="text-xs">保存后添加基准图</span>
                            </div>
                        )}
                    </div>
                    <div className="grid min-w-0 content-start gap-4">
                        ...
                        <label className="grid gap-1.5 text-sm">
                            ...
                            <Input.TextArea value={draft.description} onChange={(event) => setDraft((current) => ({ ...current, description: event.target.value }))} autoSize={{ minRows: 3, maxRows: 5 }} placeholder="一句话说明它在故事中的作用" />
                        </label>
                        {kind === "clues" ? (
                            <label className="grid gap-1.5 text-sm">
                ...
                <section className="border-t border-border pt-5">
                    <div className="mb-3">
                        ...
                        <p className="mt-1 text-xs leading-5 text-muted-foreground">这些规则会被分镜与生成提示词稳定引用。</p>
                    </div>
                    <div className="grid gap-3 sm:grid-cols-2">
                        {(["visualIdentity", "styling", "colorPalette", "consistencyRules"] as const).map((key, index) => (
                            <label key={key} className="grid gap-1.5 text-sm">
                                ...
                                <Input.TextArea value={draft.profile[key]} onChange={(event) => setDraft((current) => ({ ...current, profile: { ...current.profile, [key]: event.target.value } }))} autoSize={{ minRows: 2, maxRows: 4 }} />
                            </label>
                        ))}
                    </div>
                </section>
                ...
                {kind === "characters" ? (
                    <section className="border-t border-border pt-5">
                        <div className="mb-3">
                            ...
                            <p className="mt-1 text-xs leading-5 text-muted-foreground">留空音色 ID 时沿用后台默认音频模型配置。</p>
                        </div>
                        <div className="grid gap-3 sm:grid-cols-[minmax(140px,0.8fr)_110px_minmax(220px,1.2fr)]">
                            ...
                            />
                            <Input
                                value={draft.voiceProfile.instructions}
                                onChange={(event) => setDraft((current) => ({ ...current, voiceProfile: { ...current.voiceProfile, instructions: event.target.value } }))}
                ...
                <section className="border-t border-border pt-5">
                    ...
                    {!asset ? <div className="mt-4 rounded-lg border border-dashed border-border bg-muted/25 px-4 py-6 text-center text-sm text-muted-foreground">先创建资产，再上传或生成参考图。</div> : null}
                    {asset && !references.length ? <div className="mt-4 rounded-lg border border-dashed border-border bg-muted/25 px-4 py-6 text-center text-sm text-muted-foreground">还没有参考图，可上传已有设定或生成候选图。</div> : null}
                    {references.length ? (
                        <Image.PreviewGroup>
