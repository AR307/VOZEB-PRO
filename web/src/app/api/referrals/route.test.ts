...
describe("GET /api/referrals", () => {
    ...
    it("builds the share link from the current public IP instead of a loopback setting", async () => {
        const response = await GET(new Request("http://192.168.1.20:3000/api/referrals"));

        expect(response.status).toBe(200);
        expect(mocks.getReferralCenter).toHaveBeenCalledWith("user-one", "http://192.168.1.20:3000");
    });
});
