...
- `/create` 媒体创作记录的“重新编辑”必须只恢复原始需求、稳定参考素材、用户显式 Skill、模型和生成偏好，不自动发送；“再次生成”必须创建新的请求、用户消息、助手消息和 Run，保留原结果并复用该轮公开输入快照，不能偷换成失败重试或覆盖原轮。下载、引用、复制、失败任务重试和 Canvas/短剧交接继续沿用真实业务链路。
