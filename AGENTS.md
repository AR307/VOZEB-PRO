...
- `/create` 图片与视频 Result Card 禁止为了视觉统一设置固定大宽高或最小高度。主媒体先使用后端有效 `width/height`，缺失时读取图片 `naturalWidth/naturalHeight` 或视频 `videoWidth/videoHeight`，再回退真实比例 metadata 与安全默认值，并在有界最大尺寸内等比计算 `renderedWidth × renderedHeight`；媒体、预览容器和可见 Card 必须同宽同高，不得用 `cover` 拉伸或让媒体只占 Card 一角。结果组使用 shrink-to-fit，只有操作栏真实放不下时才允许比媒体略宽。图片与视频统一只按实际 ready 且有真实 URL 的结果数组判断多结果：`results.length <= 1` 时“更多生成结果”及切换器 DOM 必须完全不存在，`results.length > 1` 时才渲染，并保留全部成功结果和当前选中结果的真实操作。
- `/create` 媒体创作记录只有失败轮次显示“编辑重试”，点击后恢复原始需求、稳定参考素材、用户显式 Skill、模型和生成偏好，但不自动发送；成功轮次禁止显示“重新编辑”或“再次生成”，只允许复制公开提示词，并保留下载、打开和引用等真实结果操作。图片与视频必须使用同一套状态规则，内部执行提示词不得作为复制内容。
- `/create` 输入框的“优化提示词”必须调用后台默认文本模型并将结果回填为可编辑草稿，不自动发送；请求必须防重复计费，并沿用后台模型与渠道的真实能力配置，禁止额外写死输出 token 上限。用户继续编辑或发送后，迟到的优化响应不得覆盖新内容；只有用户最终发送的公开草稿才能进入可见消息和“复制提示词”操作。
