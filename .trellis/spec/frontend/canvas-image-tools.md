...
- 分层链路不执行本地像素切割、裁片上传或本地透明预览；bbox 和 focusPoints 只作为文本识别结果写入图片任务提示词，不能作为图片上游参考图。
- 人物、普通主体和电商图的“分层”统一进入本链路；独立“消除背景”工具才继续使用下方本地语义分割契约。
...
- 识别节点直接创建 loading 图片节点，元素和背景任务使用 `Promise.allSettled` 同时启动；上游成功后写回同一节点，上游失败时结束 loading 并记录可见错误或待检查状态，不伪造本地成功结果。
- 用户点击“智能分层”后必须立即显示“正在分层分析”；该全局提示只覆盖源图准备和一次视觉分析请求，并在分析成功或失败时立即清理。分析成功后的元素与背景生成进度只由各自节点展示，不能让全局提示持续到全部生成任务结束。
...
- 不写死元素数量、轮询次数、重试次数或输出上限；识别返回多少元素就同时处理多少元素。每项任务必须有独立稳定身份。
- 任务状态未知时保留原任务身份并进入既有人工检查/恢复路径，不自动创建第二个同类上游任务。
- 图片上游返回的透明结果落盘前必须保持源图尺寸和原始坐标，不得二次缩放、裁切或重采样；空透明图或不可读结果按该元素失败处理。
- 只有独立“消除背景”允许使用 `splitSubjectAndBackgroundDataUrl` 和本地 Worker，不创建图片生成任务；“分层”必须创建图片任务。
...
| 某个元素上游失败 | 该元素节点结束 loading 并显示明确失败或待检查状态，其他元素继续处理，不丢弃已成功结果 |
| 上游元素结果不可读、空透明或尺寸不符 | 该元素任务标记失败，不能用本地裁切、占位图或重绘图冒充结果 |
| 背景补全失败 | 元素结果保留，背景节点显示失败状态和重试入口 |
| 请求状态未知 | 沿用原任务 ID 查询/人工检查，不自动重复计费 |

### 5. Good / Bad Cases

- Good：一次视觉识别返回 20 个元素；立即创建 20 个独立 loading 图片节点，随后并发发出 20 个透明任务，每个任务都引用同一张完整主图，再发出一个背景补全任务。
- Good：某个元素上游精修失败时，对应节点显示明确失败或待检查状态，其他成功元素仍可独立移动、保存和下载。
...
- Runtime/API 测试断言同一完整主图 URL 被传给每个元素任务，背景任务使用独立蒙版；元素失败会结束 loading 且不会生成本地伪结果。
...
- 浏览器测试必须阻塞一次视觉分析响应，断言“正在分层分析”在请求挂起时可见、响应放行后消失，且后续元素与背景节点继续生成。
